    ...
    <div className="h-auto pb-4 bg-slate-200 rounded-xl mt-4 shadow-lg hover:shadow-slate-400 transition-all duration-300  flex flex-col cursor-pointer relative gap-2">
      ...
      <button>
        {checkInInterestList(interestList, product) ? (
          <BsBookmarkHeartFill
            className={`text-2xl text-red-600 transition-all duration-500 absolute top-0 right-1`}
            onClick={handleRemoveLike}
          />
        ) : (
          <BsBookmarkHeart
            className={` text-2xl  hover:text-red-600 transition-all duration-300 absolute top-0 right-1 text-red-400`}
            onClick={handleLike}
          />
        )}
      </button>
