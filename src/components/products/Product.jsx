  ...
  return (
    <div className="h-auto pb-4 bg-slate-200 rounded-xl mt-4 shadow-lg hover:shadow-slate-400 transition-all duration-300  flex flex-col cursor-pointer relative gap-2">
      ...
      <button onClick={handleLike}>
        {checkInInterestList(interestList, product) ? <BsBookmarkHeartFill className={`text-2xl text-red-600 transition-all duration-500 absolute top-0 right-2`} /> : <BsBookmarkHeart className={` text-2xl  hover:text-red-600 transition-all duration-300 absolute top-0 right-2 text-red-400`} />}
      </button>
      <NavLink to={product.to} key={product.id} product={product}>
        ...
        {/* free transport */}
        <div className='w-[6.3rem] md:w-32 flex  mt-4 px-2 bg-slate-300 rounded-lg mr-2 md:py-1  text-slate-600 items-center'>
          <img src={product.logo} className='w-3 h-3 md:w-4 md:h-4  object-cover ' alt="phone" />
          <span className='text-slate-400  text-[9px] md:text-xs mr-3'>ارسال رایگان</span>
        </div>
        {/* title */}
        <div className='w-full px-4 mt-4 text-slate-600 text-[10px] md:text-sm'>
          <span> {product.name}</span>
        </div>
        {/* add btn */}
        <div className='w-full flex justify-between text-[9px] md:text-xs px-4 mt-2 text-slate-600'>
