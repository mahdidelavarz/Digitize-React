...
const Links = ({ iconStyle, backStyle, width }) => {
    ...
    const routes = [
        ...
        { title: 'تلفن همراه', path: '/phone', icon: <HiOutlineDeviceMobile className={iconStyle} /> },
        { title: 'لپ تاپ', path: '/laptop', icon: <MdOutlineLaptopMac className={iconStyle} /> },
        { title: 'ساعت هوشمند', path: '/watch', icon: <IoWatchOutline className={iconStyle} /> },
        { title: 'هدفون و هندزفری', path: '/', icon: <ImHeadphones className={iconStyle} /> },
        { title: 'علاقمندی', path: '/interests', icon: <BsHeartHalf className={iconStyle} /> }
    ]
    return (
        <>
            {routes.map((item) => {
                return <li className={`${width} h-12  text-lg text-slate-600 lg:px-4 cursor-pointer flex items-center hover:bg-gradient-to-l from-slate-200 rounded-md transition-all duration-300`} key={item.title}>
                    ...
                    <NavLink to={item.path} className='w-full flex text-base md:text-sm lg:text-base'>
                        {item.title}
                    </NavLink>
                </li>
            })}
