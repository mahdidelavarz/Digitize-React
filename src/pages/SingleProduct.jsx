...
const SingleProduct = () => {
  ...
  return (
    <Layout>
      <section className="w-full col-span-12 md:col-span-9 h-auto  relative grid grid-cols-12 md:grid-rows-12 bg-white py-2 px-6 rounded-lg items-center z-20">
        {/* ----------------------------items image--------------------------- */}
        <div className="h-full col-span-12 md:col-span-7 lg:col-span-4 relative flex items-center flex-col justify-evenly z-30">
          ...
          <img src={product.image} alt={product.name} className="w-64 object-cover md:mt-2" />
        ...
        <div className="col-span-12  md:col-span-5 lg:col-span-4  flex flex-col md:h-full md:justify-evenly">
          <h1 className=" text-base mt-8 md:text-base text-slate-500">{product.name}</h1>
          ...
          <div className="w-full h-36 flex flex-col items-end mt-7 p-2  overflow-hidden mb-4 rounded-md lg:hidden border-2 border-solid">
            ...
            <span className="flex w-full justify-start flex-row-reverse text-sm text-rose-600 cursor-pointer mt-4 ">ادامه
              مطلب</span>
          </div>
          {/* <!-- !...................................Attributes  (Desktop)............................................... --> */}
        ...
        <div className="h-full col-span-12 lg:col-span-4 flex flex-col items-center md:px-6 md:bg-slate-100 py-4 rounded-xl justify-between ">
          ...
          <div className='fixed w-full h-[4.3rem] bg-slate-100 bottom-2 rounded-3xl  p-3 z-50 md:hidden'>
            <div className='w-full h-full flex justify-between items-center px-10 rounded-2xl bg-rose-600'>
              ...
              <span className="text-white  w-auto  text-xl lg:text-base xl:text-lg">{product.price.toLocaleString('fa-IR')} تومان</span>
            </div>
          </div>
          {/* desktop add to cart */}
          <div className="w-full md:flex flex-col justify-end items-end mt-6 static  bg-slate-100 z-30 hidden ">
            <span className="text-rose-700  w-auto  text-xl lg:text-base xl:text-lg">{product.price.toLocaleString('fa-IR')} تومان</span>
            <button onClick={() => handleAddToCart(product)} className="  w-full py-2  bg-rose-500 rounded-md mt-2  text-white  lg:text-base text-lg">{checkInCart(cart, product) ? 'در سبد خرید موجود است' : 'افزودن به سبدخرید'}</button>
          </div>
        </div>
      </section>
    </Layout>
